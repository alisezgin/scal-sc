...
ifeq ($(strip $(foreach prefix,$(NO_LOAD),\
    $(findstring $(join ^,$(prefix)),\
                 $(join ^,prodcon-sc-disconn-ms.target.mk)))),)
  include prodcon-sc-disconn-ms.target.mk
endif
ifeq ($(strip $(foreach prefix,$(NO_LOAD),\
    $(findstring $(join ^,$(prefix)),\
                 $(join ^,sc-disconn-ms.target.mk)))),)
  include sc-disconn-ms.target.mk
endif

quiet_cmd_regen_makefile = ACTION Regenerating $@
cmd_regen_makefile = cd $(srcdir); ./build/gyp/gyp_main.py -fmake --ignore-environment "--toplevel-dir=." "--depth=." scal-sc.gyp
Makefile: glue-sc.gypi scal-sc.gyp common-sc.gypi
