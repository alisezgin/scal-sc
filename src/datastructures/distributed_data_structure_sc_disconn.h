...
#include "datastructures/balancer_sc_disconnected.h"
...
// the following code is commented out: 19/11/2015
// reason: it is doing a round of checks across other queues; 
// this should be avoided for the SC case.
...
// the above code is commented out: 19/11/2015
// the following code is added: 19/11/2015
// we only check the local queue;
// this is the only required operation for pure SC.
//  size_t i;
  start = balancer_->get_id();
  if (backend_[start]->get(item))
    return true;
  else
    return false;
// the above code is added: 19/11/2015
}
